...
def pushforward_sample_steps(seed, step, pushforward):
    ...
    unroll_steps_idx = torch.multinomial(
        probs, num_samples=1, replacement=True, generator=rng
    ).item()
...
def pushforward_preprocess(features, target_pos, unroll_steps):
    """Compute the target acceleration for the given pushforward steps."""
...
def pushforward_fn(features, target_positions, unroll_steps, forward_function):
    """Apply pushforward to the input data."""
...
def pushforward_integrate(
    normalized_acceleration, position_sequence, normalization_stats, boundaries, pbc=True
):
    """
    ...
    norm_st = normalization_stats
    acceleration = (normalized_acceleration * norm_st["std"]) + norm_st["mean"]

    # Use an Euler integrator to go from acceleration to position, assuming dt = 1.
    most_recent_position = position_sequence[:, -1]
    if pbc:
        most_recent_velocity = wrap_displacement(
            (most_recent_position - position_sequence[:, -2]), boundaries
        )
